...
class App extends React.Component {
  ...
  updateCards() {
    if (this.state.weatherCards.length > 0) {
      this.state.weatherCards.map((currCard) => {
        fetch(
          `https://api.openweathermap.org/data/2.5/weather?q=${currCard[0]}&appid=${APIKEY}`
        )
          .then((response) => {
            if (response.ok) {
      ...
      fetch(
        ...
        .then((data) => {
          ...
          this.state.weatherCards.map((item) => {
            extraCard = this.state.weatherCards.filter(
              (item) => item[0] === data.name
            );
          });
          if (input.value !== "" && extraCard.length < 1) {
            if (this.state.weatherCards.length > 2) {
              this.state.weatherCards.pop();
            }
            ...
            newDate.getHours() < 10
              ? (currentTime =
                  "0" + newDate.getHours() + ":" + newDate.getMinutes())
            ...
            this.state.globalTempConvert === true
              ? (tempToWeatherCards = Math.round(
                  Math.round(data.main.temp - 273.15) * 1.8 + 32
                ))
            ...
            if (this.state.globalTempConvert === true) {
              this.setState({
                ...
                feelsLikeTemp: Math.round(
                  Math.round(data.main.feels_like - 273.15) * 1.8 + 32
                ),
              });
            } else {
              this.setState({
  ...
  moreInfoRequest(currentData) {
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${currentData[0]}&appid=${APIKEY}`
    )
      .then((response) => {
        if (response.ok) {
          return response.json();
  ...
  deleteCards(card) {
    ...
    if (
      this.state.weatherCards.indexOf(card) === 2 ||
      this.state.weatherCards.indexOf(card) === 1
    ) {
  ...
  convertToCels(e) {
    if (this.state.tempConvert === true) {
      ...
      this.setState(
        { tempConvert: false, temperature: celcius, feelsLikeTemp: feelslike },
        () => {
          localStorage.setItem("card", JSON.stringify(this.state));
        }
      );
